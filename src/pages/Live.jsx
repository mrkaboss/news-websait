...
export default function Live() {
  ...
  useEffect(() => {
    ...
    script.onerror = () => setError("Failed to load PeerJS — Please check your internet connection");
    document.head.appendChild(script);
  }, []);

  const startLive = async () => {
    if (!peerReady) { setError("Please wait — System is initializing..."); return; }
    try {
      setLoading(true);
      setError("");
      ...
      peer.on("error", (err) => { setError("An error occurred: " + err.message); setLoading(false); });
    } catch (err) {
      setError(err.name === "NotAllowedError" ? "Please allow camera and microphone access in your browser!" : "An error occurred: " + err.message);
      setLoading(false);
    }
  };

  const watchLive = async () => {
    if (!peerReady) { setError("Please wait..."); return; }
    if (!hostId.trim()) { setError("Please enter the presenter's Live ID!"); return; }
    try {
      setLoading(true);
      setError("");
      const peer = new window.Peer();
      ...
      peer.on("open", () => {
        ...
        if (!call) { setError("This Live ID is not available"); setLoading(false); return; }
        ...
        call.on("error", () => { setError("Connection failed — Please check the Live ID"); setLoading(false); });
        call.on("close", () => { setError("This live stream has ended"); setJoined(false); });
      });
      peer.on("error", () => { setError("Live ID not found or stream has ended"); setLoading(false); });
    } catch (err) {
      ...
      <div style={{ padding: "28px 24px", maxWidth: "860px", margin: "0 auto" }}>
        ...
        {!mode && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minHeight: "70vh", justifyContent: "center", animation: "fadeUp 0.5s ease" }}>
            ...
            <p style={{ color: "#aaa", fontSize: "14px", textAlign: "center", marginBottom: "12px" }}>
              No account required — Completely free
            </p>
            <p style={{ color: "#777", fontSize: "12px", textAlign: "center", marginBottom: "48px" }}>
              Go Live → Share Live ID → People watch you live instantly
            </p>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", width: "100%", maxWidth: "480px" }}>
              <div className="mode-card" onClick={() => setMode("broadcaster")} style={{ background: "#0e0e1a", border: "1px solid #1e1e30", borderRadius: "24px", padding: "36px 24px", textAlign: "center", cursor: "pointer" }}>
                ...
                <p style={{ color: "#555", fontSize: "12px", lineHeight: 1.5 }}>Host a live stream — Share the link with viewers</p>
              </div>
              ...
              <div className="mode-card mode-card-blue" onClick={() => setMode("viewer")} style={{ background: "#0e0e1a", border: "1px solid #1e1e30", borderRadius: "24px", padding: "36px 24px", textAlign: "center", cursor: "pointer" }}>
                <div style={{ fontSize: "44px", marginBottom: "14px" }}>📺</div>
                <p style={{ fontWeight: 900, fontSize: "17px", marginBottom: "6px" }}>Watch Live</p>
                <p style={{ color: "#555", fontSize: "12px", lineHeight: 1.5 }}>Join a stream — Use the Live ID provided to you</p>
              </div>
            </div>
          </div>
        )}
        ...
        {mode === "broadcaster" && (
          <div style={{ animation: "fadeUp 0.4s ease" }}>
            <h2 style={{ fontSize: "22px", fontWeight: 900, marginBottom: "20px" }}>🎥 Your Studio</h2>
            ...
            <div style={{ position: "relative", borderRadius: "20px", overflow: "hidden", background: "#000", aspectRatio: "16/9", maxWidth: "720px", border: joined ? "2px solid #EF4444" : "2px solid #1e1e30", boxShadow: joined ? "0 0 50px #EF444422" : "none", marginBottom: "16px" }}>
              ...
              {!joined && (
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "10px", color: "#666" }}>
                  <span style={{ fontSize: "48px" }}>🎥</span>
                  <p style={{ fontSize: "13px" }}>Camera preview will appear once you click "Go Live"</p>
                </div>
              )}
              {joined && (
                <>
                  <div style={{ position: "absolute", top: "14px", left: "14px", display: "flex", alignItems: "center", gap: "7px", background: "#EF4444", padding: "5px 12px", borderRadius: "50px" }}>
                    ...
                    <span style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "2px" }}>YOU ARE LIVE</span>
                  </div>
            ...
            {joined && peerId && (
              <div style={{ background: "#0e0e1a", border: "1px solid #EF444444", borderRadius: "16px", padding: "16px 20px", marginBottom: "16px", maxWidth: "720px" }}>
                <p style={{ fontSize: "11px", fontWeight: 700, color: "#EF4444", letterSpacing: "2px", marginBottom: "8px" }}>
                  📤 SHARE THIS ID WITH YOUR VIEWERS
                </p>
                ...
                <p style={{ fontSize: "11px", color: "#555", marginTop: "8px" }}>
                  People who want to watch your stream must enter this ID into the "Watch Live" section.
                </p>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", maxWidth: "720px" }}>
              {!joined ? (
                <button className="btn" onClick={startLive} disabled={loading} style={{ background: "#EF4444", color: "#fff", padding: "14px 32px", borderRadius: "14px", fontWeight: 900, fontSize: "15px", opacity: loading ? 0.7 : 1, display: "flex", alignItems: "center", gap: "8px" }}>
                  {loading ? <><div style={{ width: "16px", height: "16px", border: "2px solid #fff", borderTop: "2px solid transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> Starting...</> : "🔴 Go Live"}
                </button>
              ) : (
                <>
                  ...
                  <button className="btn" onClick={toggleCam} style={{ background: isCamOff ? "#EF4444" : "#1e1e30", color: "#fff", padding: "12px 22px", borderRadius: "12px", fontWeight: 700, fontSize: "13px" }}>
                    {isCamOff ? "📷 Turn On Camera" : "📵 Turn Off Camera"}
                  </button>
        ...
        {mode === "viewer" && (
          <div style={{ animation: "fadeUp 0.4s ease" }}>
            <h2 style={{ fontSize: "22px", fontWeight: 900, marginBottom: "20px" }}>📺 Watch Live</h2>
            ...
            <div style={{ position: "relative", borderRadius: "20px", overflow: "hidden", background: "#000", aspectRatio: "16/9", maxWidth: "720px", border: joined ? "2px solid #3B82F6" : "2px solid #1e1e30", marginBottom: "16px" }}>
              ...
              {!joined && (
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "10px", color: "#666" }}>
                  <span style={{ fontSize: "48px" }}>📺</span>
                  <p style={{ fontSize: "13px" }}>Enter the Live ID below and click "Join Stream"</p>
                </div>
              )}
              {joined && (
                <div style={{ position: "absolute", top: "14px", left: "14px", display: "flex", alignItems: "center", gap: "7px", background: "#EF4444", padding: "5px 12px", borderRadius: "50px" }}>
                  <div style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#fff", animation: "pulse 1s infinite" }} />
                  <span style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "2px" }}>STREAM LIVE</span>
                </div>
              )}
            </div>
            ...
            {!joined && (
              <div style={{ maxWidth: "720px", marginBottom: "16px" }}>
                <p style={{ fontSize: "11px", fontWeight: 700, color: "#3B82F6", letterSpacing: "2px", marginBottom: "8px" }}>ENTER PRESENTER'S LIVE ID</p>
                <input
                  type="text"
                  placeholder="Paste or type Live ID here..."
                  value={hostId}
            ...
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
              {!joined ? (
                <button className="btn" onClick={watchLive} disabled={loading} style={{ background: "#3B82F6", color: "#fff", padding: "14px 32px", borderRadius: "14px", fontWeight: 900, fontSize: "15px", opacity: loading ? 0.7 : 1, display: "flex", alignItems: "center", gap: "8px" }}>
                  {loading ? <><div style={{ width: "16px", height: "16px", border: "2px solid #fff", borderTop: "2px solid transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> Connecting...</> : "▶ Join Live"}
                </button>
              ) : (
                <button className="btn" onClick={stopStream} style={{ background: "#1e1e30", color: "#EF4444", padding: "12px 22px", borderRadius: "12px", fontWeight: 700, fontSize: "13px", border: "1px solid #EF444444" }}>
                  ⏹ Leave Stream
