...
export default function Sports() {
  ...
  useEffect(() => {
    const fetchSportsData = async () => {
      try {
        ...
        const sportsFiltered = newsArray.filter(
          item => item.category?.toLowerCase() === "sports"
        );

        setNews(sportsFiltered);
      } catch (err) {
        console.error("Sports Fetching Error:", err);
        setError(true);
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to permanently delete this sports story?")) return;
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="border-b border-gray-200 pb-6 mb-10 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-black text-gray-900 tracking-tight uppercase flex items-center gap-2">
              <span className="w-2.5 h-6 bg-red-600 rounded-sm inline-block" /> Sports Central
            </h1>
            <p className="text-gray-400 text-xs mt-1 font-semibold uppercase tracking-wider">
              {news.length} Active sports coverages
            </p>
          </div>
        ...
        {!loading && !error && news.length === 0 && (
          <div className="text-center py-24 bg-white rounded-3xl shadow-sm border border-dashed border-gray-200">
            ...
            <p className="text-gray-400 font-black text-sm uppercase tracking-wider">No active sports summaries matched this directory</p>
          </div>
        )}
              ...
              >
                ...
                <div className="h-52 w-full overflow-hidden bg-gray-100 relative">
                  <img
                    ...
                    onError={e => { e.currentTarget.src = "https://images.unsplash.com/photo-1461896836934-ffe607ba8211"; }}
                  />
                </div>
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="mt-auto pt-5 border-t border-gray-100/80 flex items-center justify-between">
                    ...
                    >
                      View full story <span className="transform group-hover:translate-x-0.5 transition-transform">→</span>
                    </Link>
                  </div>
                </div>
              </div>
